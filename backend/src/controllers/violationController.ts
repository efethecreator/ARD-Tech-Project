...
export default class ViolationController {
    ...
    static async updateViolation(req: Request, res: Response) {
        try {
            ...
            const updatedViolation = await violationModel.findByIdAndUpdate(id, updates, { new: true });
            if (!updatedViolation) {
                res.status(404).json({ message: "Violation not found" });
                return;
            }
    ...
    static async deleteViolation(req: Request, res: Response) {
        try {
            ...
            res.status(200).json({ message: "Violation deleted successfully", deletedViolation });
        } catch (error) {
